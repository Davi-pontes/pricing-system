    ...
    async handle(httpRequest: HttpRequest<any>): Promise<HttpResponse<IProduct | string>> {
        try {
            ...
            if (!validatePropsProduct.success) {
                throw new ValidationErrorProduct(validatePropsProduct.error.issues[0].message)
            }
            ...
            const dataToCreateProductIngredient = dataProduct.productIngredients as ICreateProductIngredientParams[]

            const addIngredientProductId = dataToCreateProductIngredient.map(param => ({ ...param, id_product: idProduct }));
